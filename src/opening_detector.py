...
def op_detector_2(data_folder, images_path, bound_box, two_views):
    ...
    if bound_box=="region":
        ...
        for key in images:
            ...
            cv.imwrite("../results/" + data_folder + "/skeleton/for_skeleton_" + key + ".jpg", label_op_f*255)
...
def op_detector_1(data_folder, images_path, two_views):
    ...
    for key in images:
        ...
        cv.imwrite('../results/' + data_folder + '/corners_' + key + '.jpg', overlayed_prediction[key])
    ...
    for i, im in enumerate(imgs):
        ...
        cv.imwrite("../results/" + data_folder + "/ssd_{}.jpg".format(img_names[i]),cv.cvtColor(annotated_img,cv.COLOR_BGR2RGB))
